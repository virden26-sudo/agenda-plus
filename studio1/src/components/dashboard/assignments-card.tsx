      ...
      <CardHeader>
        <div className="flex justify-between items-start">
            <div>
                ...
                <GradientIcon name="BookOpenCheck" className="h-6 w-6" />
            ...
            {loading ? (
              ...
              upcomingAssignments.map((assignment) => (
                  <div key={assignment.id} className="flex items-center gap-4 p-2 rounded-lg hover:bg-muted/50">
                      ...
                      <Badge variant={isOverdue(assignment.dueDate) ? 'destructive' : isToday(assignment.dueDate) ? 'default' : 'outline'} className="text-sm">
              ...
              <div className="flex flex-col items-center justify-center h-full text-center p-8">
                <div className="flex items-center justify-center w-16 h-16 rounded-full bg-muted mb-4">
                    <GradientIcon name="BookOpenCheck" className="h-8 w-8 text-muted-foreground" />
                </div>
                <h3 className="font-semibold text-lg mb-1">No Upcoming Assignments</h3>
                <p className="text-muted-foreground text-sm mb-4">Sync your syllabus to add assignments.</p>
              </div>
            )}
            </div>
        </ScrollArea>
