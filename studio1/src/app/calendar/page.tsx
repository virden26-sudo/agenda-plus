...
export default function CalendarPage() {
    ...
    const events = [...assignments.map(a => ({...a, type: 'Assignment'})), ...quizzes.map(q => ({...q, type: 'Quiz'}))].map(item => ({
        date: new Date(item.dueDate),
        title: item.title,
        type: item.type,
    }));

    const renderDay = (dayProps: DayProps) => {
            ...
            <Card className="lg:col-span-2">
                <CardContent className="p-0">
                    <Calendar
                        ...
                        className="w-full"
                        components={{
                            DayContent: (dayProps) => renderDay(dayProps)
                        }}
                    />
                </CardContent>
            </Card>
